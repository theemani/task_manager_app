...
def original_is_simple_lookup_func(
    ...
    while 0 <= index < len(line.leaves):
        ...
        if current.type in disallowed:
            return False
        if current.type not in {token.NAME,
                                token.DOT} or current.value == "for":
...
class StringMerger(StringTransformer, CustomSplitMapMixin):
    ...
    def _validate_msg(line: Line, string_idx: int) -> TResult[None]:
        ...
        if num_of_inline_string_comments > 1:
            return TErr(f"Too many inline string comments ({
    ...
    def do_match(self, line: Line) -> TMatchResult:
        ...
        while True:
            ...
            if is_valid_index(idx -
                              2) and (LL[idx -
                                         2].type == token.NAME or LL[idx -
                                                                     2].type in CLOSING_BRACKETS):
                continue
