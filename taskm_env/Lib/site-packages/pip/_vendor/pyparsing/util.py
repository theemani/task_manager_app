...
class __config_flags:
    ...
    def _set(cls, dname, value):
        if dname in cls._fixed_names:
            warnings.warn(f"{cls.__name__}.{dname} {cls._type_desc} is {str(
                getattr(cls, dname)).upper()}" f" and cannot be overridden", stacklevel=3, )
            return
    ...
    if len(s) > 3:
        for _, chars in itertools.groupby(s, key=is_consecutive):
            ...
            if first == last:
                ...
                ret.append(f"{escape_re_range_char(first)}{
                    sep}{escape_re_range_char(last)}")
    else:
