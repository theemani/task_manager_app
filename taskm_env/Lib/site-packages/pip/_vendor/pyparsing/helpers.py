    ...
    if useRegex:
        ...
        try:
            if all(len(sym) == 1 for sym in symbols):
                ...
                # pattern
                patt = f"[{''.join(_escape_regex_range_chars(sym)
                                   for sym in symbols)}]"
            else:
...
def infix_notation(
    ...
    for i, operDef in enumerate(op_list):
        opExpr, arity, rightLeftAssoc, pa = (
            operDef + (None,))[:4]  # type: ignore[assignment]
...
def delimited_list(
    ...
    *,
    ...
    return DelimitedList(expr, delim, combine, min, max,
